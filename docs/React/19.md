...
1. **觸發組件的 render 函數**：
   ...
   - 當某個組件的 state 或 props 改變時（例如透過 setState 或父組件傳遞新的 props），React 會重新執行該組件的 render 函數，生成新的虛擬 DOM 樹。
...
3. **決定需要更新的部分**：
   ...
   - 根據比較結果，React 會標記需要更新的組件和 DOM 節點。
...
- 如果組件使用了 React.memo 或 shouldComponentUpdate 等優化方式，React 可能會跳過某些子組件的重新渲染。
...
假設你有一個簡單的計數器組件，當點擊按鈕時，狀態改變並觸發畫面更新：
...
1. **初始渲染**：
   ...
   - React 執行 Counter 組件的 render 函數，生成虛擬 DOM（包含 `<div>`、`<p>` 和 `<button>`）。
...
當父組件的 state 改變時，React 會重新渲染父組件，並預設也會重新渲染所有的子組件，即使子組件的 props 沒有改變。這是因為 React 的預設行為是「當父組件重新渲染時，子組件也會跟著重新渲染」。

### 為什麼會這樣？

- React 的 Reconciliation 過程會從父組件開始，逐層檢查所有子組件。

- 如果子組件的 props 或 state 沒有改變，React 仍然會執行子組件的 render 函數，生成新的虛擬 DOM，然後比較是否需要更新真實 DOM。

- 雖然這種重新渲染通常不會影響性能（因為虛擬 DOM 比較很快），但在大型應用程式中，過多的重新渲染可能導致性能問題。

### 程式碼範例（展示父子組件重新渲染）：

以下是一個父組件 Parent 和子組件 Child，當父組件的狀態改變時，子組件也會重新渲染：
...
// 子組件
function Child({ name }) {
  console.log(`渲染 Child 組件，name: ${name}`);
  return <p>我是子組件，名稱：{name}</p>;
}
...
function Parent() {
  ...
  const handleClick = () => {
    setCount(count + 1); // 改變父組件狀態
  };

  console.log("渲染 Parent 組件");

  return (
    <div>
      <p>父組件計數：{count}</p>
      <button onClick={handleClick}>加 1</button>
      <Child name="小明" /> {/* 子組件，props 不變 */}
    </div>
  );
}
...
1. **初始渲染**：
   ...
   - React 渲染 Parent 組件，生成虛擬 DOM。

   - 接著渲染 Child 組件，生成子組件的虛擬 DOM。
...
2. **點擊按鈕**：
   ...
   - **Render Phase**：
     ...
     - Reconciliation 比較新舊虛擬 DOM，發現只有 Parent 的 `<p>` 文字內容改變（例如從 父組件計數：0 變成 父組件計數：1）。
...
3. **觀察結果**：
   ...
   - 在控制台中，你會看到 渲染 Parent 組件 和 渲染 Child 組件，name: 小明 都被印出，證明即使 Child 的 props 沒變，它仍然被重新渲染。

### 如何避免不必要的子組件重新渲染？

為了優化性能，你可以使用以下方法避免不必要的子組件重新渲染：

1. **使用 React.memo**：

   - React.memo 是一個高階組件（HOC），可以讓函數式組件只在 props 改變時重新渲染。
...
1. **使用 useMemo 或 useCallback**：
   ...
   - 如果父組件傳遞的是複雜的物件或函數作為 props，可以使用 useMemo 或 useCallback 確保這些 props 的引用不變。
...
const Child = memo(({ name, onClick }) => {
  console.log(`渲染 Child 組件，name: ${name}`);
  return <p onClick={onClick}>我是子組件，名稱：{name}</p>;
});
...
4. **setState 觸發的重新渲染** 會導致父組件及其子組件重新執行 render，即使子組件的 props 沒變。

5. 使用 React.memo、useMemo 或 useCallback 可以避免不必要的子組件重新渲染，提升性能。

### 實作建議

- 如果你正在開發 React 應用程式，建議在開發初期就考慮使用 React.memo 來優化子組件渲染。

- 使用 Chrome 的 React Developer Tools，可以檢視哪些組件被重新渲染，幫助你找出性能瓶頸。
