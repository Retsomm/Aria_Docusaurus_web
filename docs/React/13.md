...
React 組件本質上就是函式，因此你需要熟悉 JavaScript 的函式寫法：
...
- **組件（Components）**：React 的基本單元，可以是函式組件或類組件。
- **Props 和 State**：用於傳遞資料和管理組件狀態。
