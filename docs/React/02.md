...
2. **提供狀態**：使用 Provider 將狀態提供給子組件。

3. **消費狀態**：在需要使用狀態的組件中透過 useContext 獲取狀態。
...
- ThemeProvider 負責提供狀態給所有子組件。

- useTheme 是一個自訂 Hook，方便在任何組件中獲取主題狀態。
...
2. **提供 Store**：在應用程式根組件中使用 Provider。

3. **連接到組件**：使用 useSelector 和 useDispatch 存取和更新狀態。
...
2. **使用 Store**：在組件中直接調用 store 的 Hook。
...
- useCounterStore 是一個 Hook，直接在組件中存取狀態和方法。
...
2. **提供 RecoilRoot**：在應用程式根組件中包裹 RecoilRoot。
...
2. **連接到組件**：使用 observer 包裝組件以響應狀態變化。
