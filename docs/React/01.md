...
React Router 是一個用於 React 應用程式的路由管理庫。它允許你在不同的 URL 路徑下渲染不同的組件，實現類似傳統多頁面網站的導航效果，但實際上仍然是單頁應用程式。React Router 的核心概念是根據瀏覽器的 URL 動態渲染對應的組件，讓使用者感覺像是在切換頁面。
...
1. **BrowserRouter**：
   ...
   - 所有路由相關的組件都必須包在 `<BrowserRouter>` 內。

2. **Routes**：

   - 用來定義所有的路由規則，取代了舊版的 `<Switch>`（v6 改進）。

   - 每個 `<Route>` 定義了一個路徑（path）和對應的組件（element）。

3. **Route**：

   - 定義特定的路徑和渲染的組件。例如，path="/" 表示當 URL 為根路徑時，渲染 `<Home />` 組件。
...
打開瀏覽器，訪問 http://localhost:3000 ，你應該會看到「首頁」內容。點擊導航列中的「關於」或「聯繫我們」，頁面會切換到對應的組件內容。
...
- `<Outlet>` 是用來渲染子路由的組件，放在父組件中，子路由的內容會在這裡顯示。
...
- **與 React 高度整合**：無需額外配置，與 React 組件無縫整合。
...
4. **懶加載（Lazy Loading）**：對於大型應用程式，使用 React 的 lazy 和 Suspense 來延遲載入路由組件，提升效能。
