...
在 React 中，**副作用 (Side Effect)** 指的是在渲染組件 (Component) 時，執行一些與渲染無直接關係的操作。例如：
...
這些操作不直接影響組件的渲染結果，但會影響應用程式的狀態或其他外部資源。React 提供了 **useEffect Hook** 來處理這些副作用，讓你可以在適當的時機執行它們。

簡單來說，**Effect** 就是 React 用來管理副作用的機制，確保副作用不會干擾組件的渲染流程。

---

## React Component Function 中的副作用

在 React 的函數組件中，副作用通常不應該直接寫在組件的頂層，因為組件函數會在每次渲染時執行。如果直接在組件函數中執行副作用，可能導致以下問題：
...
- 每次組件渲染，document.title 都會被重新設定。

- 如果有其他副作用（如 API 請求），可能會導致多次不必要的請求，影響效能。

**解決方案**：使用 useEffect Hook 來控制副作用的執行時機，避免在組件頂層直接執行。

---

## useEffect 初探

useEffect 是 React 提供的一個 Hook，用來在函數組件中處理副作用。它允許你指定副作用的執行時機，並在需要時執行清理工作。
...
- **第二個參數**：依賴陣列 (dependency array)，控制副作用何時執行。如果陣列中的值改變，副作用會重新執行。
  ...
  - 如果依賴陣列為空 (\[\])，副作用只在組件首次渲染時執行一次。

  - 如果省略依賴陣列，副作用會在每次渲染時都執行。

### 範例：使用 useEffect 修改頁面標題

以下是一個簡單的 useEffect 範例，用來在組件渲染後修改頁面標題：
...
- useEffect 中的回呼函數在組件渲染完成後執行（即 DOM 更新後）。
...
在 React 中，每次組件渲染都會產生一個新的 **effect 函式**。這意味著 useEffect 內的回呼函數會捕捉當前渲染時的 **props** 和 **state** 值。這種行為確保了副作用的邏輯與當前渲染的狀態一致。
...
- 每次 count 改變，組件重新渲染，產生一個新的 useEffect 回呼函數。
...
useEffect 允許你返回一個 **清理函數 (cleanup function)**，用來在副作用重新執行或組件卸載 (unmount) 前清理資源。每次渲染都會產生一個新的清理函數，對應當前的 effect 函數。
...
- **組件重新渲染**：當依賴陣列中的值改變時，React 會先執行上一次的清理函數，再執行新的 effect 函數。

- **組件卸載**：當組件從 DOM 中移除時，React 會執行最後一次的清理函數。
...
- useEffect 返回的清理函數在組件卸載時執行，清除計時器，防止記憶體洩漏。

- 因為依賴陣列為空 \[\]，effect 和清理函數只在組件掛載和卸載時各執行一次。
