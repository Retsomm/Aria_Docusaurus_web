...
- 物件透過 \__proto_\_（不建議直接使用）或 Object.getPrototypeOf() 存取其原型。
...
5. 使用 \__proto_\_ 或 Object.getPrototypeOf() 確認實例的原型是否指向 Person.prototype。

#### 注意事項：

- 不要直接操作 \__proto_\_，因為它是內部屬性，建議使用 Object.getPrototypeOf() 或 Object.setPrototypeOf()。
