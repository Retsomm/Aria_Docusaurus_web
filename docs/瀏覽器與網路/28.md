---
title: 什麼是瀏覽器的渲染引擎？
description: "瀏覽器的渲染引擎（Rendering Engine）是瀏覽器核心的一部分，負責將 HTML、CSS 和 JavaScript 轉換成使用者在螢幕上看到的網頁畫面。它解析網頁的原始碼（HTML 和 CSS），建構 DOM（文件物件模型）和 CSSOM（CSS 物件模型），並最終將這些內容渲染為視覺化的頁面。"
keywords: [渲染引擎, 瀏覽器]
---
