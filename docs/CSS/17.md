...
description: "傳統 CSS（也叫原生 CSS 或 Vanilla CSS）是網頁樣式的基礎語言，而 Tailwind CSS 則是一種「公用程式優先」（utility-first）的 CSS 框架，它讓你直接在 HTML 元件中使用預定義的類別（class）來套用樣式，而不用寫一大堆自訂 CSS 規則。"
