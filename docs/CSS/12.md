...
description: "z-index 是一個 CSS 屬性，用來控制元素在 z 軸（垂直於螢幕的深度方向）上的堆疊順序。而 Stacking Context（堆疊上下文） 則是瀏覽器在渲染頁面時，用來決定元素如何在 z 軸上層疊的上下文環境。簡單來說，z-index 只有在 Stacking Context 內才有效，且不同堆疊上下文..."
