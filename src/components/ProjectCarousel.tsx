...
export default function ProjectCarousel(): React.ReactElement {
  const projects: Project[] = [
    ...
    },
    {
      ...
      description:
        '這是一個使用react與material UI製作的部落格介面，以此網站致敬影集Silicon Valley，目前只有簡單的切換主題、按喜歡、RWD等等，待更新發文功能。',
      image: pied_piper,
    ...
    },
    {
      title: '水星領航員介紹',
      description:
        '這是一個使用Bootstrap與Skrollr製作的網頁，介紹「水星領航員」這部日本動漫，在火星上打造如地球威尼斯的生活，人們在那邊邂逅美好的相遇，也延伸到spaceX，Elon Musk對於宇宙探險以及火星殖民的願景。',
      image: ARIA,
  ...
  const handleFocus = (e: React.FocusEvent<HTMLElement>) => {
    const slideElement = (e.target as HTMLElement).closest('.slick-slide');
  ...
  return (
    <div className={styles.carouselContainer}>
      <div style={{ width: '100%' }} onFocus={handleFocus}>
        <Slider {...settings}>
                ...
                <div className={styles.cardContent}>
                  ...
                  <div className={styles.links}>
                    {project.links.map((link, linkIndex) => (
                      <Link key={linkIndex} to={link.url} className={styles.link} target="_blank" rel="noopener noreferrer">
                        {link.label}
                      </Link>
                    ))}
                  </div>
      ...
      <style>{`
        ...
        :global(.slick-prev) { left: 10px !important; }
        :global(.slick-next) { right: 10px !important; }
        :global(.slick-prev:before), :global(.slick-next:before) { color: white !important; opacity: 1 !important; }
        :global(.slick-dots) { bottom: -35px !important; }
      `}</style>
    </div>
  );
}
