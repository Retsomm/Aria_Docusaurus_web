...
function SearchPageContent() {
  ...
  const algoliaHelper = algoliaHelperRef.current;
  // 用 ref 確保 result handler 永遠能存取最新的 dispatcher 和 processSearchResultUrl
  const dispatcherRef = useRef(searchResultStateDispatcher);
  dispatcherRef.current = searchResultStateDispatcher;
  const processSearchResultUrlRef = useRef(processSearchResultUrl);
    ...
    const handler = ({results: {query, hits, page, nbHits, nbPages}}: any) => {
      ...
      const items: SearchResultItem[] = matchedHits.map(
        ({
          ...
          // titles.pop() 可能是 undefined（所有 hierarchy 層級都是 null）
          // 此時 fallback 到原始 hierarchy 欄位，從最深層往上找第一個非空值
          const rawTitle = ['lvl6','lvl5','lvl4','lvl3','lvl2','lvl1','lvl0']
            .map((k) => rawHierarchy[k])
            .find((v) => v != null) ?? url;
          return {
            title: titles.pop() ?? rawTitle,
            url: processSearchResultUrlRef.current(url),
  ...
  return (
    <Layout>
      ...
      <div className="container margin-vert--lg">
        ...
        {searchResultState.hasMore && !searchResultState.loading && (
