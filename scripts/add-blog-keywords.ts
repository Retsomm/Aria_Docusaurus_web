 ...
 *
 * Run: node scripts/add-blog-keywords.ts
 */
...
function insertKeywords(content: string, keywords: string[]): string {
  ...
  if (blockTagsMatch) {
    const end = (blockTagsMatch.index || 0) + blockTagsMatch[0].length;
